...
private:
  ...
  public:
	...
	ebbrt::kprintf("%s: error: unable to load model\n" , __func__);
	  ...
	  ebbrt::kprintf("%s ", llama_token_to_piece(ctx, new_token_id).c_str());
	  ...
	  ebbrt::kprintf("%s : failed to eval, return code %d\n", __func__, 1);
      ...
      ebbrt::kprintf("%s: decoded %d tokens in %.2f s, speed: %.2f t/s\n",
