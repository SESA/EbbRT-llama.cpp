...
extern void eassert(char* s, int line, bool b);
#endif
...
#define GGML_MALLOC(size)      eggml_malloc(size)
#define GGML_CALLOC(num, size) eggml_calloc(num, size)
...
#ifdef _EBBRT_
  eassert(__FILE__, __LINE__, info->n_dims <= GGML_MAX_DIMS);
  eassert(__FILE__, __LINE__, 0 <= info->type && info->type < GGML_TYPE_COUNT);

  for (uint32_t i = 0; i < info->n_dims; ++i) {
    eassert(__FILE__, __LINE__, info->ne[i] > 0);
  }

  // prevent overflow for total number of elements
  eassert(__FILE__, __LINE__, INT64_MAX/info->ne[1] > info->ne[0]);
  eassert(__FILE__, __LINE__, INT64_MAX/info->ne[2] > info->ne[0]*info->ne[1]);
  eassert(__FILE__, __LINE__, INT64_MAX/info->ne[3] > info->ne[0]*info->ne[1]*info->ne[2]);
...
#ifdef _EBBRT_
static bool gguf_fread_el(unsigned char * file, void * dst, size_t size, size_t * offset) {
  memcpy(dst, file+(*offset), size);
  *offset += size;
  return true;
}
static bool gguf_fread_str(unsigned char * file, struct gguf_str * p, size_t * offset) {
    p->n    = 0;
    p->data = NULL;
    ...
    if (p->n == SIZE_MAX) {
      char buffer[100];
      snprintf(buffer, 100, "%s: invalid string length (%u)\n", __func__, p->n);
      eprint(buffer);
      eassert(__FILE__, __LINE__, false);
    }
...
#ifdef _EBBRT_
struct gguf_context * gguf_init_from_buffer(unsigned char *file, struct gguf_init_params params) {
  char buffer [100];
  unsigned char *beginfile = file; // store a copy
  ...
  // check the magic before making allocations
  {
    ...
    for (uint32_t i = 0; i < sizeof(magic); i++) {
      snprintf(buffer, 100, "magic[%d] = %c\n", i, magic[i]);
      eprint(buffer);
      eassert(__FILE__, __LINE__, magic[i] == GGUF_MAGIC[i]);
  ...
  // read the header
  {
    ...
    eassert(__FILE__, __LINE__, ctx->header.version != 1);
  ...
  // read the kv pairs
  {
      ...
      case GGUF_TYPE_ARRAY:
	{
	  ...
	  case GGUF_TYPE_BOOL:
		...
		snprintf(buffer, 100, "%s: array size is too large (%" PRIu64 ")\n", __func__, kv->value.arr.n);
		eprint(buffer);
		eassert(__FILE__, __LINE__, false);
	  ...
	  case GGUF_TYPE_ARRAY:
	  default:
	    eassert(__FILE__, __LINE__, false && "invalid type");
    ...
    if (!ok) {
      snprintf(buffer, 100, "%s: failed to read key-value pairs\n", __func__);
      eprint(buffer);
      eassert(__FILE__, __LINE__, ok);
  ...
  if (ctx->header.n_tensors > 0) {
	...
	if (strcmp(info->name.data, ctx->infos[j].name.data) == 0) {
	  snprintf(buffer, 100, "%s: duplicated tensor name %s\n", __func__, info->name.data);
	  eprint(buffer);
	  eassert(__FILE__, __LINE__, false);
      ...
      if (!ok) {
	snprintf(buffer, 100, "%s: failed to read tensor info\n", __func__);
	eprint(buffer);
	eassert(__FILE__, __LINE__, false);
  ...
  // we require the data section to be aligned, so take into account any padding
  {
    ...
    if (offset_pad != 0) {
      ...
      // restart from beginning + offset
      file = beginfile + offset;
    }
  }
  ...
  // compute the total size of the data section, taking into account the alignment
  {
	...
	snprintf(buffer, 100, "%s: tensor '%s' of type %d (%s) number of elements (%" PRId64 ") is not a multiple of block size (%d)\n", __func__, info->name.data, (int)info->type, ggml_type_name(info->type), ne, ggml_blck_size(info->type));
	eprint(buffer);
	eassert(__FILE__, __LINE__, false);
  ...
  if (params.ctx != NULL) {
    ...
    *params.ctx = ggml_init(pdata);
    ...
    if (!params.no_alloc) {
      ...
      if (!ok) {
	snprintf(buffer, 100, "%s: failed to read tensor data\n", __func__);
	eprint(buffer);
	eassert(__FILE__, __LINE__, false);
